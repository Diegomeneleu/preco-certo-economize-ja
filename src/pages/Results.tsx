      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <div className="container mx-auto px-4 py-6 flex-1 flex items-center justify-center">
          <div className="text-center">
            <p className="text-white">Sem dados de comparação disponíveis</p>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="container mx-auto px-4 py-6 flex-1">
        <div className="card-gradient rounded-lg shadow-lg p-6 mb-6">
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-secondary/20 text-secondary mb-3">
              <Check size={32} />
            </div>
            <h2 className="text-2xl font-bold text-white">Recomendação</h2>
            <p className="text-white/90">O mercado mais econômico é:</p>
          </div>
          
          <div className="text-center mb-6">
            <h3 className="text-3xl font-bold text-secondary">{cheapestMarket.marketName}</h3>
            <p className="text-xl font-semibold mt-2 text-white">R$ {cheapestMarket.totalPrice.toFixed(2)}</p>
          </div>
          
          <div className="bg-green-950/30 border border-green-500/20 rounded-lg p-4">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-white/90">Economia total:</p>
                <p className="text-lg font-semibold text-white">R$ {savingsAmount.toFixed(2)}</p>
              </div>
              <div className="text-right">
                <p className="text-white/90">Em percentual:</p>
        ...
        <div className="card-gradient rounded-lg shadow p-4 mb-6">
          <h3 className="text-lg font-semibold mb-3 text-white">Comparação completa</h3>
          <div className="space-y-3">
            {marketComparisons.map((market, index) => (
              <div 
                key={market.marketId}
                className={`flex justify-between items-center p-3 rounded-lg ${
                  index === 0 ? 'bg-green-950/30 border border-green-500/20' : 'bg-primary/20 border border-primary/30'
                }`}
              >
                <div>
                  <p className={`font-medium text-white ${index === 0 ? 'text-secondary' : ''}`}>
